...
from src.data import FUEL_COLUMNS, get_price, load_component_history, load_components, load_history, load_official_prices, load_sources
from src.hybrid import reconstruct_price, reconstruction_audit, scenario_estimate
from src.modeling import build_trend_chart, create_lagged_data, forecast_fuel
...
class DataTests(unittest.TestCase):
    ...
    def test_required_project_files_exist(self):
        for name in ["data/nairobi_price_history.csv", "data/current_nairobi_price.csv", "data/price_components.csv", "data/nairobi_component_history.csv", "data/sources.csv", "data/price_revisions_2026.csv"]:
    ...
    def test_effective_periods_are_valid(self):
        self.assertTrue((self.history["Effective_To"] >= self.history["Effective_From"]).all())

    def test_every_history_row_has_registered_source(self):
        self.assertTrue(set(self.history["Source_ID"]).issubset(set(self.sources["Source_ID"])))
        self.assertTrue(self.sources["URL"].str.startswith("https://").all())

    def test_official_spot_checks(self):
        jul_2022 = self.history.loc[self.history["Cycle"].eq(pd.Timestamp("2022-07-01"))].iloc[0]
        apr_2026 = self.history.loc[self.history["Cycle"].eq(pd.Timestamp("2026-04-01"))].iloc[0]
        may_2026 = self.history.loc[self.history["Cycle"].eq(pd.Timestamp("2026-05-01"))].iloc[0]
        self.assertEqual((jul_2022.Super_Petrol, jul_2022.Diesel, jul_2022.Kerosene), (159.12, 140.0, 127.94))
        self.assertEqual((apr_2026.Super_Petrol, apr_2026.Diesel, apr_2026.Kerosene), (197.60, 196.63, 152.78))
        self.assertEqual((may_2026.Super_Petrol, may_2026.Diesel, may_2026.Kerosene), (214.25, 232.86, 191.38))
    ...
    def test_component_detail_reconciles_to_epra_totals(self):
        totals = load_components().groupby("Fuel")["KES_Per_Litre"].sum().round(2).to_dict()
        self.assertEqual(totals, {"Diesel": 162.91, "Kerosene": 146.93, "Super Petrol": 177.32})
    ...
    def test_component_history_is_real_linked_and_reconciled(self):
        ...
        self.assertEqual(frame.groupby("Fuel").size().to_dict(), {"Diesel": 11, "Kerosene": 11, "Super Petrol": 11})
        self.assertTrue(frame["PDF_URL"].str.startswith("https://www.epra.go.ke/").all())
        self.assertLessEqual(reconstruction_audit(frame)["Calculated_Error"].abs().max(), 0.02)

    def test_known_annex_row_reconstructs_exact_price(self):
        frame = load_component_history()
        row = frame.loc[frame["Effective_From"].eq(pd.Timestamp("2025-06-15")) & frame["Fuel"].eq("Diesel")].iloc[0]
        self.assertEqual(reconstruct_price(row), 162.91)

    def test_cost_scenario_changes_only_declared_inputs(self):
        frame = load_component_history()
        row = frame.loc[frame["Effective_From"].eq(pd.Timestamp("2025-06-15")) & frame["Fuel"].eq("Super Petrol")].iloc[0]
    ...
    def test_lags_use_only_past_values(self):
        ...
        self.assertAlmostEqual(first["Rolling_3"], self.history.iloc[:3]["Super_Petrol"].mean())
...
class CalculatorTests(unittest.TestCase):
    ...
    def test_trip_calculation_includes_traffic_allowance(self):
        ...
        self.assertEqual(result, {"base_litres": 10.0, "litres": 11.0, "cost": 2200.0})

    def test_invalid_values_are_rejected(self):
        for function, args in [(cost_for_litres, (0, 200)), (litres_for_budget, (-1, 200)), (trip_estimate, (100, 0, 200))]:
...
class ModelTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.history = load_history()
        cls.results = {column: forecast_fuel(cls.history, column) for column in FUEL_COLUMNS.values()}

    def test_all_fuels_return_finite_deterministic_forecasts(self):
        for column, result in self.results.items():
            for value in [result.prediction, result.lower, result.upper, result.mae, result.rmse, result.baseline_mae]:
    ...
    def test_holdout_is_separate_from_selection(self):
        ...
        self.assertEqual(result.selection_points + result.validation_points + 24, len(create_lagged_data(self.history, "Super_Petrol")))
    ...
    def test_chart_connects_last_observation_to_forecast(self):
        ...
        self.assertEqual(chart.loc[pd.Timestamp("2026-08-01"), "Forecast"], result.prediction)

    def test_model_features_exclude_same_cycle_external_values(self):
        self.assertNotIn("USD_KES", app.FEATURE_COLUMNS)
        self.assertNotIn("Crude_Oil", app.FEATURE_COLUMNS)
        self.assertEqual(set(app.FEATURE_COLUMNS), {"Month_num", "Month_sin", "Month_cos", "Lag_1", "Lag_2", "Rolling_3"})
